...
TEST(TestShape, test_circle)
{
    ...
    // just default
    auto b2c = (b2CircleShape *)c.ToBox2DShape({0, 0}, 0);
    ...
    // change radius
    c.Radius(10);
    b2c = (b2CircleShape *)c.ToBox2DShape({0, 0}, 0);
    ...
    // change offset
    b2c = (b2CircleShape *)c.ToBox2DShape({3, 2}, 0);
    ASSERT_FLOAT_EQ(b2c->m_radius, 10);
    ...
    // change rotate
    c.Center({2, 0});
    b2c = (b2CircleShape *)c.ToBox2DShape({3, 2}, M_PI / 6);
    ASSERT_FLOAT_EQ(b2c->m_radius, 10);
...
TEST(TestShape, test_polygon)
{
    ...
    // just default
    auto b2p = (b2PolygonShape *)p.ToBox2DShape({0, 0}, 0);
    ...
    // change center
    p.Center({0.5, 0.5});
    b2p = (b2PolygonShape *)p.ToBox2DShape({0, 0}, 0);
    ...
    // change offset
    b2p = (b2PolygonShape *)p.ToBox2DShape({1, 2}, 0);
    ...
    // change rotate
    b2p = (b2PolygonShape *)p.ToBox2DShape({1, 2}, M_PI / 4);
    auto tmp = sqrtf(2) / 2;
    ASSERT_TRUE(Vec2In(b2p->m_vertices, 4, {1, 2}));
